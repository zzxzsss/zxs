...
local PhantomForcesWindow = Library:NewWindow("Abyss Miner")
local KillingCheats = PhantomForcesWindow:NewSection("Options")
local ShopSection = PhantomForcesWindow:NewSection("Shop")
...
local pickaxes = {
    ...
    {damage = 450, name = "Chasm Smasher", multiplier = 1.2, hitboxType = "Heavy"},
    {damage = 90, name = "[Relic]Spiral Drill of Abyss", multiplier = 0.2, hitboxType = "Drill"},
}
...
local slowFallFactor = 0.15 -- default 15%
...
local function teleportToAllNestedParts()
	local hrp = getHRP()
...
local function autoTeleportStones()
...
-- GUI Toggles and Sliders

KillingCheats:CreateToggle("Auto HitBox", function(value)
	autoHitBoxEnabled = value
	if value then task.spawn(autoHitBox) end
end)

KillingCheats:CreateToggle("Auto Sell Inventory", function(value)
	autoSellEnabled = value
	if value then task.spawn(autoSellInventory) end
end)

KillingCheats:CreateToggle("Auto Surface", function(value)
	autoLayer1Enabled = value
	if value then task.spawn(autoTeleportLayer1) end
end)

KillingCheats:CreateToggle("Auto Cave2Stone", function(value)
	autoTeleportEnabled = value
	if value then task.spawn(autoTeleportStones) end
end)

KillingCheats:CreateToggle("Auto Quartz Stone", function(value)
	autoQuartzStoneEnabled = value
	if value then task.spawn(autoTeleportQuartzStone) end
end)

KillingCheats:CreateToggle("Auto Magma Rock", function(value)
	autoMagmaEnabled = value
	if value then task.spawn(autoTeleportMagma) end
end)

KillingCheats:CreateToggle("Auto Heated Magma Rock", function(value)
	autoHeatedMagmaEnabled = value
	if value then task.spawn(autoTeleportHeatedMagma) end
end)

KillingCheats:CreateToggle("Auto Remove Dead Parts", function(value)
	autoRemoveDeadPartsEnabled = value
	if value then task.spawn(autoRemoveDeadParts) end
end)

KillingCheats:CreateToggle("Slow Fall", function(value)
...
KillingCheats:CreateSlider("Fall Speed (%)", 0, 50, 15, false, function(value)
...
TeleportSection:CreateDropdown("Teleport To", teleportOptions, 1, function(selected)
	local hrp = getHRP()
	if not hrp then
...
ShopSection:CreateDropdown("Buy Pickaxe", shopPickaxeNames, 1, function(selectedPickaxeName)
...
-- Added RankUp in Shop Section
ShopSection:CreateButton("Open RankUp GUI", function()
	local args = {
		workspace:WaitForChild("Map"):WaitForChild("Layer 1"):WaitForChild("Npcs"):WaitForChild("Karl"),
		workspace:WaitForChild("Map"):WaitForChild("Layer 1"):WaitForChild("Npcs"):WaitForChild("Karl"):WaitForChild("HumanoidRootPart"):WaitForChild("Dialogue")
	}
	game:GetService("ReplicatedStorage"):WaitForChild("RemoteEvent"):WaitForChild("RankUpGui"):FireServer(unpack(args))
end)

-- Add Remove Fog Toggle
TeleportSection:CreateToggle("Remove Fog", function(value)
	if value then
...
TeleportSection:CreateToggle("Full Bright", function(value)
	if value then
		...
		-- Reset to default values (you can adjust these as needed)
...
local ESPSection = PhantomForcesWindow:NewSection("ESP")
...
local oreColors = {
	...
	["Obsidian"] = Color3.fromRGB(75, 0, 130),          -- Indigo (new color for obsidian)
}
...
local function runESP()
	while espEnabled do
		...
		local function addESPToOreFolder(oreFolderName)
			...
			-- Special case for Obsidian โ€” deeper nested:
			if oreFolderName == "Obsidian" then
				...
				if oreFolder then
					...
					if heatedMagma and heatedMagma:FindFirstChild("Amount") then
						...
						oreFolder = oreFolder -- fallback if no HeatedMagmaRock.Amount
					end
				end
			else
...
ESPSection:CreateToggle("ESP On/Off", function(value)
	espEnabled = value
	if espEnabled then
		task.spawn(runESP)
	else
...
ESPSection:CreateDropdown("Select Ore for ESP", oresForESP, 1, function(selected)
...
TeleportSection:CreateButton("Save Position A", function()
	local hrp = getHRP()
	if hrp then
...
TeleportSection:CreateButton("Save Position B", function()
	local hrp = getHRP()
	if hrp then
...
TeleportSection:CreateToggle("Toggle Walk Between A & B", function(value)
